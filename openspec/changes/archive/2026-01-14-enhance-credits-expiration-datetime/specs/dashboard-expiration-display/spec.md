...
## Purpose
Enhance the expiration datetime display in dashboard Credits Period sections to show full datetime with UTC+7 timezone.
...
- **THEN** the "Expires" field SHALL display format: `DD/MM/YYYY HH:mm:ss (UTC+7)`
- **AND** the datetime SHALL be converted to UTC+7 timezone before display
- **EXAMPLE**: `15/01/2026 23:59:59 (UTC+7)`
...
- **THEN** the "Expires" field SHALL display format: `DD/MM/YYYY HH:mm:ss (UTC+7)`
- **AND** the datetime SHALL be converted to UTC+7 timezone before display
- **EXAMPLE**: `20/01/2026 23:59:59 (UTC+7)`
...
- **AND** both SHALL show the `(UTC+7)` suffix

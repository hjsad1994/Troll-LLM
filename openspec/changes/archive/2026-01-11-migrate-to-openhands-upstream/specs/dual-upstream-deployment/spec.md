...
**Depends on**:
- `billing` spec: For credit deduction logic
- `request-logging` spec: For logging requests to both upstreams

**Related to**:
- `ohmygpt-failover-endpoint` spec: Provides redundancy alongside failover
- `configure-dual-domain-deployment` change: Implements this architectural vision
...
1. **Port Assignments**:
   - Port 8004: OpenHands upstream (goproxy-openhands)
   - Port 8005: OhMyGPT upstream (goproxy-ohmygpt)
   - These ports are internal to Docker network only

2. **SSL Certificates**:
   - Use Certbot with `--expand` flag to add `chat2.trollllm.xyz` to existing certificate
   - Alternative: Request separate certificate if needed

3. **Resource Considerations**:
   - Running two GoProxy instances will increase memory usage
   - Monitor VPS resources and adjust if needed
   - Each instance maintains its own connection pool (keepalive 32)

4. **User Selection**:
   - Users choose upstream by selecting the appropriate base URL
   - No automatic failover or load balancing between upstreams
   - Frontend MAY expose this choice in UI (future enhancement)

5. **Monitoring**:
   - Log files should distinguish between upstreams
   - Health checks should monitor both services independently
   - Consider separate Discord webhook alerts for each upstream

# Spec: Migration Script for Credits (Rate 1000 → 2500)
...
- Script SHALL only migrate users with `migration: false`
- Credit division SHALL use exact ratio 2.5 (2500/1000)
- Migration SHALL be atomic per user (either full success or no change)
- Script SHALL continue processing after individual user failures
- Migration logs SHALL only be created for successful migrations
- Admin users MAY be excluded with optional `--exclude-admins` flag (optional feature)

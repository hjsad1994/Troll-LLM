  ...
  return (
    <div>
      <header className="mb-8">
        <h1 className="text-3xl font-bold text-slate-100">🧌 Troll-Keys</h1>
      </header>
      ...
      <div className="flex justify-between items-center mb-5 flex-wrap gap-3">
        ...
        <button onClick={() => setCreateModal(true)} className="btn btn-primary">+ Add Troll-Key</button>
      </div>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="text-center py-12 text-slate-500">
                ...
                No Troll-Keys yet. Add your first key!
              </td></tr>
            ) : (
              keys.map((key) => {
                ...
                return (
                  <tr key={keyId} className="border-b border-dark-border hover:bg-slate-700/50">
                    ...
                    <td className="px-4 py-3"><code className="text-sm">{key.maskedApiKey || '***'}</code></td>
                    ...
                    <td className="px-4 py-3 space-x-2">
                      <button onClick={() => confirm('Reset status for this Troll-Key?', () => resetKey(keyId))} className="btn btn-sm btn-secondary">Reset Status</button>
                      <button onClick={() => confirm('Delete this Troll-Key?', () => deleteKey(keyId))} className="btn btn-sm btn-danger">Delete</button>
                    </td>
                  </tr>
                )
              })
      ...
      <Modal isOpen={createModal} onClose={() => setCreateModal(false)} title="Add Troll-Key">
        <form onSubmit={createKey} className="space-y-4">
          <div>
            ...
            <input type="text" value={form.id} onChange={(e) => setForm({...form, id: e.target.value})} required placeholder="e.g., troll-1" className="input" />
          </div>
          <div>

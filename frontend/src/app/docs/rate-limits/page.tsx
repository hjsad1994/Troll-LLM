    ...
    <div className="rounded-xl border border-gray-200 dark:border-white/5 overflow-hidden mb-6">
      ...
      <div className="relative">
        ...
        <button
          ...
          className="absolute top-3 right-3 p-2 rounded-lg bg-gray-700/50 hover:bg-gray-600/50 text-slate-400 hover:text-white transition-all"
        >
          {copied ? (
...
export default function RateLimitsPage() {
        ...
        <main className="flex-1 min-w-0 px-6 lg:px-12 py-12">
          <div className="max-w-3xl">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-slate-500 mb-8">
              ...
              <span>/</span>
              <span className="text-gray-600 dark:text-slate-400">{t.docs.sidebar.rateLimits}</span>
            </div>

            {/* Title */}
            <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-4">{t.docsRateLimits.title}</h1>
            <p className="text-lg text-gray-700 dark:text-slate-400 mb-8">
              {t.docsRateLimits.description}
            </p>
            ...
            {/* Rate Limit Headers */}
            <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">{t.docsRateLimits.rateLimitHeaders}</h2>
            <p className="text-gray-700 dark:text-slate-400 mb-6">
              {t.docsRateLimits.rateLimitHeadersDesc}
            </p>

            <div className="space-y-4 mb-8">
              <div className="p-4 rounded-xl bg-gray-50 dark:bg-white/[0.02] border border-gray-300 dark:border-white/5">
                <code className="text-emerald-600 dark:text-emerald-400 font-mono text-sm">X-RateLimit-Limit</code>
                <p className="text-gray-700 dark:text-slate-400 text-sm mt-2">Maximum number of requests allowed per minute</p>
              </div>
              <div className="p-4 rounded-xl bg-gray-50 dark:bg-white/[0.02] border border-gray-300 dark:border-white/5">
                <code className="text-emerald-600 dark:text-emerald-400 font-mono text-sm">X-RateLimit-Remaining</code>
                <p className="text-gray-700 dark:text-slate-400 text-sm mt-2">Number of requests remaining in the current window</p>
              </div>
              <div className="p-4 rounded-xl bg-gray-50 dark:bg-white/[0.02] border border-gray-300 dark:border-white/5">
                <code className="text-emerald-600 dark:text-emerald-400 font-mono text-sm">X-RateLimit-Reset</code>
                <p className="text-gray-700 dark:text-slate-400 text-sm mt-2">Unix timestamp when the rate limit window resets</p>
              </div>
            </div>

            {/* 429 Response */}
            <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">{t.docsRateLimits.handlingRateLimits}</h2>
            <p className="text-gray-700 dark:text-slate-400 mb-6">
              {t.docsRateLimits.handlingDesc} <code className="px-1.5 py-0.5 rounded bg-amber-100 dark:bg-white/10 text-amber-700 dark:text-amber-400 font-mono text-sm">429 Too Many Requests</code> {t.docsRateLimits.response}
            </p>
  ...
  "error": {
    ...
    "code": "rate_limit_exceeded"
            ...
            <Warning>
              {t.docsRateLimits.warning}
            </Warning>

            {/* Best Practices */}
            <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">{t.docsRateLimits.bestPractices}</h2>
            <ul className="space-y-3 mb-8">
              <li className="flex items-start gap-3">
                ...
                <span className="text-gray-700 dark:text-slate-300 text-sm">Implement exponential backoff when receiving 429 errors</span>
              </li>
              <li className="flex items-start gap-3">
                <div className="w-5 h-5 rounded-full bg-emerald-200 dark:bg-emerald-500/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                  <svg className="w-3 h-3 text-emerald-700 dark:text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <span className="text-gray-700 dark:text-slate-300 text-sm">Monitor rate limit headers to avoid hitting limits</span>
              </li>
              <li className="flex items-start gap-3">
                <div className="w-5 h-5 rounded-full bg-emerald-200 dark:bg-emerald-500/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                  <svg className="w-3 h-3 text-emerald-700 dark:text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <span className="text-gray-700 dark:text-slate-300 text-sm">Use request queuing for batch operations</span>
              </li>
              <li className="flex items-start gap-3">
                <div className="w-5 h-5 rounded-full bg-emerald-200 dark:bg-emerald-500/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                  <svg className="w-3 h-3 text-emerald-700 dark:text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">

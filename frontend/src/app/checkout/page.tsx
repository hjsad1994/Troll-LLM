...
import { paymentDisabled, PAYMENT_ENABLED } from '@/lib/i18n'
...
function PaymentDisabledMessage({ language }: { language: 'en' | 'vi' }) {
  const msg = paymentDisabled[language]
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-gradient-to-br dark:from-gray-950 dark:to-black flex items-center justify-center p-6">
      <div className="max-w-md w-full text-center">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-3">{msg.title}</h1>
        <p className="text-gray-600 dark:text-gray-400 mb-8">{msg.message}</p>
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <a
            href="https://discord.gg/Prs3RxwnyQ"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-[#5865F2] text-white font-semibold hover:bg-[#4752C4] transition-colors"
          >
          ...
          <Link
            href="/"
            className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl border border-gray-300 dark:border-white/10 text-gray-700 dark:text-gray-300 font-medium hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
          >
            {msg.backToHome}
          </Link>
        </div>
...
function CheckoutContent() {
  ...
  // Show payment disabled message if payment is not enabled
  if (!PAYMENT_ENABLED) {
    return <PaymentDisabledMessage language={language} />
  }
